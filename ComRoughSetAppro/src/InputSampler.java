...
public class InputSampler<K,V> extends Configured implements Tool  {
  ...
  public interface Sampler<K,V> {
     ...
     * input data.
     */
	List<String> getSample(TextInputFormat inf, Job job) 
  ...
  public static class SplitSampler<K,V> implements Sampler<K,V> {
    ...
    public List<String> getSample(TextInputFormat inf, Job job) 
        throws IOException, InterruptedException {
    	
      List<InputSplit> splits = inf.getSplits(job);
      List<String> samples = new ArrayList<String>(numSamples);
      ...
      for (int i = 0; i < splitsToSample; ++i) {
        ...
        RecordReader<LongWritable,Text> reader = inf.createRecordReader(
            splits.get(i), samplingContext);
        reader.initialize(splits.get(i), samplingContext);
        while (reader.nextKeyValue()) {
          samples.add(ReflectionUtils.copy(job.getConfiguration(),
                                           reader.getCurrentValue(), null).toString());
          ++records;
  ...
  public static class IntervalSampler<K,V> implements Sampler<K,V> {
     ...
     */
    @SuppressWarnings("unchecked") // ArrayList::toArray doesn't preserve type
    public List<String> getSample(InputFormat<K,V> inf, Job job) 
        throws IOException, InterruptedException {
      List<InputSplit> splits = inf.getSplits(job);
  ...
  public static <K,V> void writePartitionFile(Job job, Sampler<K,V> sampler) 
    ...
    final TextInputFormat inf = 
        //ReflectionUtils.newInstance(job.getInputFormatClass(), conf);
    		ReflectionUtils.newInstance(TextInputFormat.class, conf);
    int numPartitions = job.getNumReduceTasks();
    List<String> samples = sampler.getSample(inf, job);
    ...
    TreeMap<Text, Integer> sampleResults=new SamplesDeal().map(samples,comparator);
    int sum=0;
    for(Integer i:sampleResults.values()){
    	sum+=i;
    }
    ...
    //int last = -1;
    int stepSize=sum/numPartitions;
    int i=0;
    for(Text key:sampleResults.keySet()){
    	i+=sampleResults.get(key);
    	if(i>=stepSize){
    		writer.append(key, nullValue);
    		i=0;
    		numPartitions--;
    	}
    	if(numPartitions<=1) break;
    }
